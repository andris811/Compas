...
<template>
  <div class="">
    <div class="my-10 container w-full text-center" v-for="question in questions">
      <div class="flex flex-row w-auto h-screen">
        ...
        <div class="basis-2/3 bg-darkblue rounded-r-xl">
          ...
          <div v-if="question.content === 'Where do you want to go?'">
            ...
            <select
              id="countries"
              class="bg-white border border-yellow-400 text-darkblue hover:bg-yellow text-lg rounded-lg focus:ring-yellow-300 focus:border-yellow-500 block w-auto p-2.5 dark:bg-yellow-700 dark:border-yellow-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-yellow-500 dark:focus:border-yellow-500 m-auto"
          ...
          <div v-if="question.content === 'How many people do you want to travel with?'">
            <div class="grid grid-cols-2">
              <div v-for="answer in question.answers">
                <button
                  type="button"
                  class="w-3/4 my-5 bg-white text-darkblue hover:bg-yellow border border-yellow-400 hover:bg-yellow focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center dark:border-yellow-300 dark:text-yellow-300 dark:hover:text-white dark:hover:bg-yellow-400 dark:focus:ring-yellow-900"
                >
                  {{ answer.content }}
                </button>
              </div>
          ...
          <div v-if="question.content === 'What kind of activities would you like to join?'">
            ...
            <select
              id="countries"
              class="bg-white border border-yellow-400 text-darkblue text-lg rounded-lg hover:bg-yellow focus:ring-yellow-300 focus:border-yellow-500 block w-auto p-2.5 dark:bg-yellow-700 dark:border-yellow-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-yellow-500 dark:focus:border-yellow-500 m-auto"
            >
              <option v-for="answer in question.answers">
                {{ answer.content }}
              </option>
    ...
    <div class="flex justify-end container -mt-20">
      <button
        type="button"
        class="focus:outline-none text-darkblue bg-yellow hover:bg-darkblue hover:text-yellow focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-lg px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900"
        @click="submit()"
      >
        Submit
      </button>
